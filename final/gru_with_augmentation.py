...
def batch_generator(xIn, yIn, num_batch_size):
    while True:
        ...
        outputX2 = xIn[num_list,:,:,:,:]
        outputX2 = np.flip(outputX2,axis=1)
        outputX = np.concatenate((outputX,outputX2),axis=0)
        num_list2 = random.sample(range(0, int(num_batch_size * 2)), num_batch_size)
        outputX[num_list2,:,:,:,:] = np.flip(outputX[num_list2,:,:,:,:],axis=3)
        yOut = yIn[num_list,:]
        yOut = np.hstack((yOut,np.zeros((num_batch_size, 1))))
        yOut = np.vstack((yOut,np.zeros((num_batch_size, 4))))
        yOut[num_batch_size:,-1] = 1
...
def batch_test_generator(testXIn, testYIn, num_batch_size):
    for index3 in range(0,testYIn.shape[0],num_batch_size):
        ...
        yOut = np.hstack((yOut,np.zeros((yOut.shape[0], 1))))
...
np.savetxt(f"results_{modeltype}_{repeat}_{dateTimeStr}.csv", results, delimiter=",")
